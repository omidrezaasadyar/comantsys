...
- **Access Control (RBAC)** — Filament Shield: roles + permissions + policies enforcing **per-operation** access. `super_admin` is **not** an automatic gate bypass (`define_via_gate => false` in `config/filament-shield.php`), so every policy must test `hasRole('super_admin')` explicitly — see `TaskPolicy` / `PortalRequestPolicy`. **User management** via `UserResource` — create/edit users and assign roles entirely in-panel (no terminal).
...
- **Daily Tasks / Follow-ups (پیگیری‌ها)** — `tasks` table + `TaskResource` under the «امور روزانه» nav group. TWO-WAY delegation model: any user holding `Create:Task` can assign a follow-up to any user; assignee (`user_id`) does the work, creator (`created_by`) delegates. Model auto-manages `done_at` via a `saving` hook (stamped on first completion, cleared if un-done). UI wording is deliberately non-imperative («پیگیری» not «وظیفه»). THREE intentional owner-scopes, each answering a different question: dashboard widget = `user_id` (tasks assigned TO me), resource list = `user_id OR created_by` (mine + ones I handed out), report page = `created_by` (did my delegated work get done). All scopes fail closed for a null user (`1=0` guard). Inline `ToggleColumn`/`TextInputColumn` for quick done/note, guarded by `->disabled(fn($record) => !can('update',$record))` since editable columns do NOT auto-check policies. Dashboard "today's tasks" rows open a bespoke modal Action (`mountAction('completeTask', {task: id})`) with a custom Blade info card + real Toggle/TextInput fields, scoped + policy-checked at both mount and save. Report page (`TaskReport`, custom Blade matching dashboard DNA): KPI cards + per-user breakdown (super_admin only) + detail list.
